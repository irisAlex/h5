  ...
  const formValuesChange = (changedValues, allValues) => {
    ...
    if (changedValues.applicantType?.length > 0) {
      if (changedValues.applicantType[0] === '01') {
        let newFormList = respondentInfoFormList.map((item) => {
          if (
            item.label == '单位名称' ||
            item.label == '信用代码' ||
            item.label == '机构代表人' ||
        ...
        console.log('自然人', newFormList);
        setRespondentInfoFormList(newFormList);
      }
