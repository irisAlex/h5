...
export const findDom = (el, index) => {
  ...
  if (el.type === 'uploadImg') {
    return <UploadImgDom {...el} key={el.name} />;
  }
  if (el.type === 'date') {
...
export const handleApplicantInfoFormList = (
  formList,
  applyType,
  hiddenArr = [],
) => {
...
export const handleRespondentInfoFormList = (
  formList,
  applyType,
  hiddenArr = [],
) => {
...
export const mediationRecordsInfoFormList = (formList) => {
  let newFormList = [];
  newFormList = formList.map((item) => {
    return { ...item };
  });
